...
for (let i = listOfMenuItems.length - 1; i > -1; i--) {
    ...
    if (listOfMenuItems[i].subMenuItems) {
        str += '<div class="relative group">'
        str += '<a href="#" class="font-rubik text-xl ml-4 mr-4 text-blue hover:text-rldarkblue-500 hover:cursor-pointer">' + listOfMenuItems[i].name + '</a>'
        str += '<div class="hidden group-hover:block absolute right-0 bg-rlgrey/100 shadow-md mt-2 p-2 rounded-md border-1 border-black z-40">'
...
for (let i = 0; i < listOfMenuItems.length; i++) {
    if(listOfMenuItems[i].subMenuItems){
        str += '<div class="relative text-center p-2 m-2 text-center group">'
