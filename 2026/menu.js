...
for (let i = listOfMenuItems.length - 1; i > -1; i--) {
    ...
    if (listOfMenuItems[i].subMenuItems) {
        ...
        str += '<div class="hidden group-hover:block absolute right-0 shadow-md bg-white/95 p-2 rounded-md z-40">'
...
for (let i = 0; i < listOfMenuItems.length; i++) {
    if(listOfMenuItems[i].subMenuItems){
        ...
        str += '<div class="hidden group-hover:block absolute right-0 shadow-lg bg-white/95 mt-2 p-2 rounded-md z-40">'
        for (let j = 0; j < listOfMenuItems[i].subMenuItems.length; j++) {
